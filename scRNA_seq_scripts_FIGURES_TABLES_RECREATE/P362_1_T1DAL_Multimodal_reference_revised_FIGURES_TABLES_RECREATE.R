...
load(file.path(resultDir,"mydata_postQC.RData"))
...
# Load object created above 

## Load Seurat reference data set and previously saved data
pbmc <- LoadH5Seurat(file.path(annotationDir, "pbmc_multimodal.h5seurat"))
...
#### FIGURE S5C: Map CD8 Seurat cell definitions onto Monocle data ####

# Load Monocle clustering and new seurat mapped reference data
load(file.path(resultDir,"P362-1 T1DAL cds object - postQC no MAIT cells.Rdata"))
cds_seurat_reference_CD8 <- cds_no_MAIT
load(file = file.path(resultDir, "Seurat_reference_mapped_metadata_CD8.RData"))
