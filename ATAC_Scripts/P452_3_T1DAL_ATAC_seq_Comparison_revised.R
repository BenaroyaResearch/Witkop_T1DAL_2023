...
path <- file.path(baseDir, "RAW_DATA/P452-1/Compiled_gene_lists/Giles_ATAC_NIHMS1781187-supplement-mmc3.xlsx")
...
Daniel_OCRs <- read_excel(file.path(baseDir, "/RAW_DATA/P452-1/Compiled_gene_lists/Daniel_science_41590_2022_1337_MOESM2_ESM.xlsx"),
...
TEMRA_overlaps_giles <- function(subject, query) {
  ...
  # subset giles data
  subject_data <- giles_Table_S2_EMRA %>% filter(subset_comparison == subject)
...
TEMRA_anno <- function(subject) {
  ...
  subset_data <- giles_Table_S2_EMRA %>% filter(subset_comparison == subject) %>% 
...
TEMRA_overlaps_giles <- function(subject, query) {
  ...
  subject_data <- TEMRA_overlaps_giles_all_df %>% filter(subset == subject) %>%
    #filter(padj <= 0.05)
...
#### FIGURE S15E: CALCULATE OVERLAP WITH JUST GENES UP IN TEMRA AND UP IN CD57 ####
...
TEMRA_overlaps_giles_up <- function(subject, query) {
  ...
  # subset giles data - USE DATSET ANNOTATED TO GENES
  subject_data <- TEMRA_overlaps_giles_all_up_df %>% filter(subset == subject) %>%
...
ATACseqData_P452_3_norm_counts_table_anno_long_meta <- left_join(ATACseqData_P452_3_norm_counts_table_anno_long, as.data.frame(ATACseqData_P452_3$samples))
...
#### FIGURE S15D: PLOT CD57 ATAC SIGNATURE BY EMRA ####

## Load Alice W calculate percent EM per donor and the key

sample_key <- read_xlsx(file.path(baseDir, "RAW_DATA/T1DAL_Clusters_from_T1DAL_AbATE_Placebo_8_MCs_for_Erin_2023-10-10.xlsx"), sheet=1) %>%
...
sample_key_anno <- left_join(sample_key, anno_colnames[,c("Donor_ID", "libId", "timepoint","sample_name")]) %>% filter(!is.na(libId))
...
EMRA_104 <- read_xlsx(file.path(baseDir, "RAW_DATA/T1DAL_Clusters_from_T1DAL_AbATE_Placebo_8_MCs_for_Erin_2023-10-10_SHEET_3_REORG.xlsx"))
...
#### FIGURE S15B,C:PLOT EMRA FLOW DATA ####
...
EMRA_percent <- EMRA_104_CD57 %>% filter(population !="DN") %>%
...
#### FIGURE S15A: Plot the percent of islet antigen specific CD57 cells at baseline  ####
...
Raw_data_0_wk <- read_excel(file.path(baseDir, "RAW_DATA/T1DAL_Clusters_from_T1DAL_AbATE_Placebo_8_MCs_for_Erin_2023-10-10_Raw_Data_0wk_REORG.xlsx"))
...
percent_Ag_of_CD8 <- read_excel(file.path(baseDir, "/RAW_DATA/Antigen_specific_percent_CD8_AW_2023_10_20.xlsx"))
