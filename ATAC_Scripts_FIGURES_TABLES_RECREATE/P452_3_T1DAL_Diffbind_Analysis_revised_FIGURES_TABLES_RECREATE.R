...
P452_3_sample_sheet <- read_csv(file.path(annotationDir,"02Aug2022_AAAMVKNHV_NS2000_P452-3_ATAC.csv"), skip = 19)
...
# load ATACseqData_P452_3 with blacklisted regions removed
...
load( file = file.path(resultDir, "ATACseqData_P452_3_norm_final.RData")) # loads ATACseqData_P452_3_norm
...
# filter for only those that are up and not just those that are up start
ATACseqData_P452_3_norm_db_anno_up <- ATACseqData_P452_3_norm_db_anno  %>% filter(Fold >=0)
View(ATACseqData_P452_3_norm_db_anno_up )
write.csv(ATACseqData_P452_3_norm_db_anno_up, file = file.path(resultDir, "ATACseqData_P452_3_norm_db_anno_up.csv"), row.names = FALSE)

# filter for only those that are down and now down start
ATACseqData_P452_3_norm_db_anno_down <- ATACseqData_P452_3_norm_db_anno %>% filter(Fold < 0)
write.csv(ATACseqData_P452_3_norm_db_anno_down, file = file.path(resultDir, "ATACseqData_P452_3_norm_db_anno_down.csv"), row.names = FALSE)
...
# save output for easy re-running below
#save(NK_mediated_cyto_gene_names, ATACseqData_P452_3_norm_db_anno_0.5_volcano, file = file.path(resultDir, "ATACseqData_P452_3_norm_db_anno_0.5_volcano_labels.RData"))

#### FIGURE 2A: PLOT VOLCANO PLOT OF SELECT GENES IN CD57 POS VS CD57 NEG ####

# load previously saved data objects from above
load(file.path(resultDir, "ATACseqData_P452_3_norm_db_anno_0.5_volcano_labels.RData"))
...
# save output from above for future loading 
#save(dbs, file = file.path(resultDir,"dbs_report_objects.Rdata"))
load( file = file.path(resultDir,"dbs_report_objects.Rdata")) # load dbs
...
# Export all overlapping annotated up sites
...
# THODE CODE BELOW IS LEFT FOR REFERENCE, BUT ACCESS TO BIGWIG FILES ON BIOINFORMATICS SERVER IS REQUIRED FOR THIS GRAPH 
# MANUAL EDITING OF CODE INSIDE PACKAGE IS ALSO REQUIRED, MAKING REPRODUCTION OF THIS FIGURE TIME CONSUMING 
